...
def high_resolution_cover_url(image_url):
    # Only replace the image dimensions. Replacing every "150" can corrupt
    # numeric timestamps elsewhere in Saavn's artwork filename.
    return re.sub(r'-150x150(?=\.)', '-500x500', image_url)
...
class ModuleInterface:
    ...
    def get_album_info(self, album_id: str, data={}) -> Optional[AlbumInfo]: # Mandatory if ModuleModes.download
        ...
        return AlbumInfo(
            ...
            cover_url = high_resolution_cover_url(album_data['image']), # optional
            cover_type = ImageFileTypeEnum.jpg, # optional
    ...
    def get_track_info(self, track_id: str, quality_tier: QualityEnum, codec_options: CodecOptions, data={}) -> TrackInfo: # Mandatory
        ...
        return TrackInfo(
            ...
            tags = tags,
            codec = CodecEnum.AAC,
            cover_url = high_resolution_cover_url(track_data['image']), # make sure to check module_controller.orpheus_options.default_cover_options
    ...
    def get_track_download(self, file_url, codec):
        ...
        return TrackDownloadInfo(
            ...
            file_url_headers = {'Referer': 'https://www.jiosaavn.com/'},
    ...
    def get_track_cover(self, track_id: str, cover_options: CoverOptions, data={}) -> CoverInfo: # Mandatory if ModuleModes.covers
        track_data = data[track_id] if data and track_id in data else self.get_track_json(track_id)
        cover_url = high_resolution_cover_url(track_data['image'])
